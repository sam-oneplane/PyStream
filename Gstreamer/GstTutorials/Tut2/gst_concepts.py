...
class GstConcepts:
    ...
    def __call__(self):
        ...
        # link all pipe line elements
        if not source.link(fltr) or not fltr.link(convert) or not convert.link(sink):
            self.logger.error("could not link elemets in pipeline")
            sys.exit(1)
        ...
        # Parse message (EOS or error)
        set_logger_msg(msg, self.logger)
...
def main_wrapper(_, argv):
    ...
    gst_concepts = GstConcepts(argv[1:])
    return gst_concepts()
...
if __name__ == "__main__":
    ...
    if platform.system() == 'Darwin':
        ...
        gst_concepts = GstConcepts(sys.argv[1:])
